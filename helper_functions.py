...
async def get_blood_type(answer,context):
    if answer in ['A', 'B', 'AB', 'O','not-sure']:
        context['candidate_blood_type'] = answer
        if answer == 'B':
...
async def get_is_multiorgan_candidate(answer,context):
    if answer.lower() in ['yes', 'no', 'yes-liver-kidney', 'yes-spk', 'yes-other']:
      ...
      if answer.lower() == 'yes-other' and context['organ_choice'] == 'liver':
    ...
    elif answer.lower() == 'not-sure':
      context['is_multiorgan_candidate'] = 'Not sure'
    else:
        return None
    return '1'
